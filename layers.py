...
from keras.layers import BatchNormalization, ConvLSTM2D, TimeDistributed, Reshape, RepeatVector
...
def conv_bn_relu(nb_filter, nb_row, nb_col, stride):
    def conv_func(x):
        x = Conv2D(nb_filter, (nb_row, nb_col), strides=stride, padding='same', kernel_regularizer=regularizers.l2(reg_weights))(x)
...
def time_conv_bn_relu(nb_filter, nb_row, nb_col, stride):
    def conv_func(x):
        x = TimeDistributed(Conv2D(nb_filter, (nb_row, nb_col), strides=stride, padding='same', kernel_regularizer=regularizers.l2(reg_weights)))(x)
...
def time_res_conv(nb_filter, nb_row, nb_col, stride=(1, 1)):
    def _res_func(x):
        identity = x

        a = TimeDistributed(Conv2D(nb_filter, (nb_row, nb_col), strides=stride, padding='same', kernel_regularizer=regularizers.l2(reg_weights)))(x)
        a = TimeDistributed(BatchNormalization())(a)
        a = TimeDistributed(Activation("relu"))(a)
        a = TimeDistributed(Conv2D(nb_filter, (nb_row, nb_col), strides=stride, padding='same', kernel_regularizer=regularizers.l2(reg_weights)))(a)
...
def time_dconv_bn_nolinear(nb_filter, nb_row, nb_col, stride=(2, 2), activation="relu"):
    def _dconv_bn(x):
        x = TimeDistributed(UnPooling2D(size=stride))(x)
        x = TimeDistributed(ReflectionPadding2D(padding=(int(nb_row/2), int(nb_col/2))))(x)
        x = TimeDistributed(Conv2D(nb_filter, (nb_row, nb_col), padding='valid', kernel_regularizer=regularizers.l2(reg_weights)))(x)
        x = TimeDistributed(BatchNormalization())(x)
